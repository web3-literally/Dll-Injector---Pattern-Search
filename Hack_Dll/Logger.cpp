...
string wstring2string(wstring &wcstr)
{
	// Setup converter
	setlocale(LC_ALL, "en_US.utf8");
    // UTF-8 narrow multibyte encoding
    char mbstr[MAX_LENGTH] = {0};
    wcstombs(mbstr, wcstr.c_str(), MAX_LENGTH);

	string cstr(mbstr);
	return cstr;
}

wstring string2wstring(string &mbstr)
{
	// Setup converter
	setlocale(LC_ALL, "en_US.utf8");
	wchar_t wcstr[MAX_LENGTH] = {0};
	mbstowcs(wcstr, mbstr.c_str(), MAX_LENGTH);
